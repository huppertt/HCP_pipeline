function ROSMOVE_rfMRI_conn_matrix(subjid,outfolder,force)
...
% Check if prereq met
if ~exist(fullfile(outfolder,subjid,'MNINonLinear','Results',[subjid '_MMP_CIC_timecourses.mat']),'file')
    disp([subjid ' timeseries not yet extracted. Returning'])
    return;
end

% Check if output exists; if so, compute sens and spec
if exist(fullfile(outfolder,subjid,'MNINonLinear','Results',[subjid '_rfMRI_conn_matrix.mat'])) & ~force
...
load(fullfile(outfolder,subjid,'MNINonLinear','Results',[subjid '_MMP_CIC_timecourses.mat']))
...
save(fullfile(outfolder,subjid,'MNINonLinear','Results',[subjid '_rfMRI_conn_matrix.mat']),...
