...
export sessiontype="petSessionData"
...
if [ -z "$VAR" ]; then
    ...
    eval "${thisdir}/CreateScanPET.sh ${project} ${subjid} ${sess} ${scan} ${type} ${jsess}"
fi

export xrem="${xrem}${project}/subjects/${subjid}/experiments/${sess}/scans/${scan}"


export cmd="XnatDataClientCerebro $xdc_put ${xrem}/resources/RAW_NIFTI/files/${scanfile}.nii -l ${file}"
