...
if [ -z "$VAR" ]; then
    ...
    eval "${thisdir}/CreateScanMEG.sh ${project} ${subjid} ${sess} ${scan} ${type} ${jsess}"
fi

export xrem="${xrem}${project}/subjects/${subjid}/experiments/${sess}/scans/${scan}"


export cmd="XnatDataClientCerebro $xdc_put ${xrem}/resources/LINKED_DATA/files/${scanfile} -l ${file}"
