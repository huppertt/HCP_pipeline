...
if force == 0 & exist(fullfile(outfolder,subjid,'TRUST', [subjid '_TRUST_fit_noPCA.mat']),'file')
...
if exist( fullfile(outfolder,subjid,'unprocessed','3T','TRUST' ) , 'dir' ) % Check if TRUST   data are available in unprocessed folder
...
%     system(['flirt '...
%         ' -in  ' fullfile(outfolder,subjid,'TRUST',[subjid '_3T_PCA_MSUM.nii.gz'])...
%         ' -ref ' fullfile(outfolder,subjid,'TRUST',[subjid '_3T_TRUST.nii.gz'])...
%         ' -out ' fullfile(outfolder,subjid,'TRUST',[subjid '_3T_PCA_MSUM_slice.nii.gz'])  ...
...
%     DiffPCA = niipca.vol;
    ...
    % Mask to hide all regions with low intensity in (quite selective - voxels must by >4SD for inclusion)
    %     Signal_mask = Diff(:,:,1) > 9*std( reshape(Diff(:,:,1),[],1) );
    Signal_mask = [Diff(:,:,1)./max(reshape(Diff(:,:,1),[],1 ) )];
    Signal_mask = Signal_mask > 4*std(Signal_mask(:))+mean(Signal_mask(:));
    ...
    save(fullfile(outfolder,subjid,'TRUST', [subjid '_TRUST_fit_noPCA.mat']), 'Y_v', 'T2','Signal_mask','R2');
...
%         axis square
    ...
    disp('Missing TRUST data in unprocessed folder')
end