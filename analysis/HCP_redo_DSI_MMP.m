function HCP_redo_DSI_MMP(subjid)
...
disp([subjid ' copying files.'])
    system(['cp ' fullfile(outfolder,subjid,'T1w','T1w_acpc_dc_restore_brain.nii.gz')                   ' ./T1w_brain.nii.gz']);        % ACPC T1
    system(['cp ' fullfile(outfolder,subjid,'T1w','T2w_acpc_dc_restore_brain.nii.gz')                   ' ./T2w_brain.nii.gz']);        % ACPC T2
    system(['cp ' fullfile(outfolder,subjid,'T1w','T1w_acpc_brain_mask.nii.gz')                         ' ./brain_mask.nii.gz']);       % ACPC brain mask
    system(['cp ' fullfile(outfolder,subjid,'T2FLAIR',[subjid '_3T_WMH_fuzzy_acpc.nii.gz'])             ' ./WMH.nii.gz']);              % ACPC lesion mask (this needs to be generated by Minjie Wu's code)
    system(['cp ' fullfile(outfolder,subjid,'T1w','subcortical','subcortical_all_fast_firstseg.nii.gz') ' ./subcortical.nii.gz']);      % Subcortical atlas
    system(['cp ' fullfile(outfolder,subjid,'T1w','/HCP-MMP_labels_acpc.nii.gz')                        ' ./HCP-MMP.nii.gz']);          % Custom volumetric HCP-MMP atlas - new version /HCP-MMP_labels_acpc.nii.gz
    ...
    system([fsldir '/bin/fslmaths ./dwi.nii.gz -Tmean ./dataMean.nii.gz']);   % Compute mean of DWI image to use as reference for resampling
    system([fsldir '/bin/flirt -in ./brain_mask.nii.gz -ref ./dataMean.nii.gz -out ./brain_mask_dsi.nii.gz -nosearch -applyxfm -init ' fsldir '/etc/flirtsch/ident.mat -interp nearestneighbour']); % Downsample mask to dMRI res
    system([fsldir '/bin/fslmaths brain_mask_dsi.nii.gz -kernel sphere 3 -dilM -fillh -bin brain_mask_dsi.nii.gz']); % added dilation to capture missing cortex
    
    % Resample subcortical regions to HCP-MMP atlas resolution
    disp([subjid ' resampling subcortical to atlas resolution'])
    system([fsldir '/bin/flirt -in subcortical.nii.gz -ref HCP-MMP.nii.gz -out subcortical.nii.gz -nosearch -applyxfm -init ' fsldir '/etc/flirtsch/ident.mat -interp nearestneighbour']); % Resample subcortical to atlas resolution; overwrite subcortical
        
    %% Create combined atlas

    % Load atlases
    mmp     = load_nii('./HCP-MMP.nii.gz');
    subcort = load_nii('./subcortical.nii.gz');
    ...
    sc_labels = ...
        ...
        '12' 'L_putamen'    '363'
        '13' 'L_pallidum'   '364'
        '18' 'L_amygdala'   '365'   % Note: amygdala may overwrite ribbon
        '26' 'L_accumbens'  '366'
        '49' 'R_thalamus'   '367'
        '50' 'R_caudate'    '368'
        '51' 'R_putamen'    '369'
        '52' 'R_pallidum'   '370'
        '54' 'R_amygdala'   '371'   % Note: amygdala may overwrite ribbon
        '58' 'R_accumbens'  '372'};
    
    % Loop over subcortical indices, add them to mmp atlas
    for i = 1:length(sc_labels)
       idx = subcort.img == str2num(sc_labels{i,1});
       mmp.img(idx) = str2num(sc_labels{i,3});
    end
    
    % Save new combined atlas
    disp([subjid ' saving combined atlas.'])
    save_nii(mmp, 'HCP-MMP_subcort_atlas.nii.gz');

    % Create text file that has label names (DSI Studio will use this)
    text = [mmp_labels(:,1) mmp_labels(:,2); sc_labels(:,3) sc_labels(:,2)];
