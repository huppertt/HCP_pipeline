...
Usage: PreeFreeSurferPipeline.sh [options]
  ...
  --subject=<subject>  Subject ID (required)
                       ...
                       directory for all outputs generated as path/subject
  --t1uni=<T1w images>    An @ symbol separated list of full paths to T1-weighted 
                       (T1w) structural images for the subject (required)
  --t1inv1=<T1w images>    An @ symbol separated list of full paths to T1-weighted 
                       (T1w) structural images for the subject (required)
  --t1inv2=<T1w images>    An @ symbol separated list of full paths to T1-weighted 
                       (T1w) structural images for the subject (required)
...
# ------------------------------------------------------------------------------
log_SetToolName "PreFreeSurferMP2RAGEPipeline.sh"
...
T1wInputImages_inv1=`opts_GetOpt1 "--inv1" $@`
T1wInputImages_inv2=`opts_GetOpt1 "--inv2" $@`
T1wInputImages_uni=`opts_GetOpt1 "--uni" $@`
...
log_Msg "T1wInputImages Inv 1: ${T1wInputImages_inv1}"
log_Msg "T1wInputImages Inv 2: ${T1wInputImages_inv2}"
log_Msg "T1wInputImages Unified: ${T1wInputImages_uni}"
...
# Naming Conventions
T1wImage_inv1="T1w_inv1"
T1wImage_inv2="T1w_inv2"
T1wImage="T1w"
...
T1wInputImages_inv1=`echo ${T1wInputImages_inv1} | sed 's/@/ /g'`
T1wInputImages_inv2=`echo ${T1wInputImages_inv2} | sed 's/@/ /g'`
T1wInputImages_uni=`echo ${T1wInputImages_uni} | sed 's/@/ /g'`
...
for TXw in ${Modalities} ; do
    ...
    if [ $TXw = T1w1 ] ; then
        TXwInputImages="${T1wInputImages_inv1}"
        TXwFolder=${T1wFolder}
        TXwImage=${T1wImage_inv1}
        TXwTemplate=${T1wTemplate}
    	TXwTemplate2mm=${T1wTemplate2mm}
    else 
        TXwInputImages="${T1wInputImages_inv2}"
        TXwFolder=${T1wFolder}
        TXwImage=${T1wImage_inv2}
        TXwTemplate=${T1wTemplate}
    	TXwTemplate2mm=${T1wTemplate2mm}
    fi
    OutputTXwImageSTRING=""

    if [ ! -f ${TXwInputImages[1]} ] ; then
        log_Msg "Skipping ${TXw} allignment"
        ...
        log_Msg "mkdir -p ${TXwFolder}/ACPCAlignment_${TXwImage}"
        mkdir -p ${TXwFolder}/ACPCAlignment_${TXwImage}
        ${RUN} ${HCPPIPEDIR_PreFS}/ACPCAlignment.sh \
            --workingdir=${TXwFolder}/ACPCAlignment_${TXwImage} \
            --in=${TXwFolder}/${TXwImage} \
            --ref=${TXwTemplate} \
            --out=${TXwFolder}/${TXwImage}_acpc \
            --omat=${TXwFolder}/xfms/${TXwImage}_acpc.mat \
            --brainsize=${BrainSize}

        # Brain Extraction(FNIRT-based Masking)
        log_Msg "Performing Brain Extraction using FNIRT-based Masking"
        log_Msg "mkdir -p ${TXwFolder}/BrainExtraction_FNIRTbased_${TXwImage}"
        mkdir -p ${TXwFolder}/BrainExtraction_FNIRTbased_${TXwImage}
        ${RUN} ${HCPPIPEDIR_PreFS}/BrainExtraction_FNIRTbased.sh \
            --workingdir=${TXwFolder}/BrainExtraction_FNIRTbased_${TXwImage} \
...
cp -v ${T1wInputImages_uni} ${T1wFolder}/T1w_uni.nii.gz

convert_xfm -omat ${T1wFolder}/T1w_inv2_acpc_xfminv.mat -inverse ${T1wFolder}/xfms/T1w_inv2_acpc.mat

flirt -ref ${T1wFolder}/T1w_inv21_gdc.nii.gz -in ${T1wFolder}/T1w_inv2_acpc_brain_mask.nii.gz \
    -out ${T1wFolder}/T1w_inv2_mask.nii.gz -applyxfm -init ${T1wFolder}/T1w_inv2_acpc_xfminv.mat -nosearch

flirt -ref ${T1wFolder}/T1w_inv1_acpc_brain_mask.nii.gz -in ${T1wFolder}/T1w_inv2_mask.nii.gz \
    -out ${T1wFolder}/T1w_inv2_mask.nii.gz -applyxfm -init ${T1wFolder}/xfms/T1w_inv1_acpc.mat -nosearch

fslmaths ${T1wFolder}/T1w_inv2_mask.nii.gz -mas ${T1wFolder}/T1w_inv1_acpc_brain_mask.nii.gz ${T1wFolder}/T1w_uni_acpc_mask.nii.gz

fslmaths ${T1wFolder}/T1w_uni_acpc_mask.nii.gz -bin ${T1wFolder}/T1_uni_acpc_mask.nii.gz


fslreorient2std ${T1wFolder}/T1w_uni.nii.gz ${T1wFolder}/T1w_gdc.nii.gz

flirt -ref ${T1wFolder}/T1w_inv1_acpc_brain.nii.gz -in ${T1wFolder}/T1w_gdc.nii.gz	-out ${T1wFolder}/T1w_acpc.nii.gz -applyxfm -init ${T1wFolder}/xfms/T1w_inv1_acpc.mat -nosearch

mkdir -p ${T1wFolder}/BrainExtraction_FNIRTbased

${RUN} ${HCPPIPEDIR_PreFS}/BrainExtraction_FNIRTbased.sh \
            --workingdir=${T1wFolder}/BrainExtraction_FNIRTbased \
            --in=${T1wFolder}/T1w_acpc \
            --ref=${T1wTemplate} \
            --refmask=${TemplateMask} \
...
if [ ! -f ${T2wFolder}/${T2wImage}_acpc_brain.nii.gz ] ; then

    log_Msg "No T2w image.  Skipping T1->T2 alignment"
