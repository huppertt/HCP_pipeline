...
if [ ! -f ${BiasField}.nii.gz ] ; then
   ...
   t1folder=`dirname ${T1wBrainImage}` 
   mkdir ${t1folder}/fast
...
if [ $DistortionCorrection = "TOPUP" ] ; then
    if [ ! -f ${SpinEchoPhaseEncodePositive} ] ; then
        DistortionCorrection="NONE"
        echo "Not using field map correction"
    fi
fi
...
if [ $DistortionCorrection = "FIELDMAP" ] ; then
  ...
  if [[ X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) > 1.25 ) {1}" | bc -l` = X1 || X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) < 0.75 ) {1}" | bc -l` = X1 ]] ; then
   ...
   # ${FSLDIR}/bin/flirt -interp spline -2D -in ${WD}/Scout.nii.gz -ref ${T1wImage} -omat "$WD"/Scout2T1w.mat -out ${WD}/Scout2T1w.nii.gz
    ...
    export SUBJECTS_DIR
    fsl_rigid_register -i ${WD}/Scout.nii.gz -r ${T1wImage}.nii.gz -o ${WD}/Scout2T1w.nii.gz -fslmat "$WD"/Scout2T1w.mat
    ...
    #${GlobalScripts}/epi_reg2 --epi=${WD}/Scout_brain.nii.gz --t1=${T1wImage} --t1brain=${WD}/${T1wBrainImageFile} --out=${WD}/${ScoutInputFile}_undistorted --fmap=${WD}/FieldMap.nii.gz --fmapmag=${WD}/Magnitude.nii.gz --fmapmagbrain=${WD}/Magnitude_brain.nii.gz --echospacing=${DwellTime} --pedir=${UnwarpDir}
  else
    echo "skipped"
    # register scout to T1w image using fieldmap
    SUBJECTS_DIR=${FreeSurferSubjectFolder}
    export SUBJECTS_DIR
    #${GlobalScripts}/epi_reg2 --epi=${WD}/Scout.nii.gz --t1=${T1wImage} --t1brain=${WD}/${T1wBrainImageFile} --out=${WD}/${ScoutInputFile}_undistorted --fmap=${WD}/FieldMap.nii.gz --fmapmag=${WD}/Magnitude.nii.gz --fmapmagbrain=${WD}/Magnitude_brain.nii.gz --echospacing=${DwellTime} --pedir=${UnwarpDir}
...
###### TOPUP VERSION (SE FIELDMAPS) ######
  ...
  cp ${WD}/${ScoutInputFile}_undistorted.nii.gz ${WD}/${ScoutInputFile}_undistorted2.nii.gz
 ...
 # apply Jacobian correction to scout image (optional)
  ...
  fsl_rigid_register -i ${WD}/Scout.nii.gz -r ${T1wBrainImage}.nii.gz -o ${WD}/Scout2T1w.nii.gz -fslmat "$WD"/Scout2T1w.mat 
  fsl_rigid_register -r ${WD}/Scout.nii.gz -i ${T1wBrainImage}.nii.gz -o ${WD}/temp.nii.gz -fslmat "$WD"/T1w2Scout.mat             
  ...
  #${FSLDIR}/bin/epi_reg                                 --epi=${WD}/Scout_brain.nii.gz --t1=${T1wImage} --t1brain=${WD}/${T1wBrainImageFile} --out=${WD}/${ScoutInputFile}_undistorted
  ${FSLDIR}/bin/flirt -in ${WD}/Scout_brain.nii.gz -ref ${WD}/${T1wBrainImageFile} -usesqform -omat ${WD}/${ScoutInputFile}_undistorted.mat
