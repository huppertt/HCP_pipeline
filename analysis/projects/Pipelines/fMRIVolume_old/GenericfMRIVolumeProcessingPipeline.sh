...
# ${FSLDIR}/bin/fslswapdim "$fMRIFolder"/"$OrigTCSName".nii.gz -x -y z "$fMRIFolder"/"$OrigTCSName".nii.gz

log_Msg "Coef  are: ${GradientDistortionCoeffs}"
...
if [ $fMRIScout = "NONE" ] ; then
  ...
  #  ${FSLDIR}/bin/fslswapdim "$fMRIFolder"/"$OrigScoutName".nii.gz -x -y z "$fMRIFolder"/"$OrigScoutName".nii.gz
fi

if [ ! -f "$fMRIFolder"/"$NameOffMRI"_gdc_warp ] ; then
...
if [ ! $GradientDistortionCoeffs = "NONE" ] ; then
    ...
    log_Msg "Using previous distortion correction"
fi

#Create fake "Scout" if it doesn't exist
if [ $fMRIScout = "NONE" ] ; then
  ${RUN} ${FSLDIR}/bin/fslroi "$fMRIFolder"/"$NameOffMRI"_gdc "$fMRIFolder"/"$ScoutName"_gdc 0 1
fi


if [ ! -f ${fMRIFolder}/Movement_RelativeRMS.txt ] ; then
...
log_Msg "Using motion correction"
