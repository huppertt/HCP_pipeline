...
dualscene=[HCProot '/pipeline/projects/Pipelines/PostFix/PostFixScenes/ICA_Classification_DualScreenTemplate.scene'];
singlescene=[HCProot '/pipeline/projects/Pipelines/PostFix/PostFixScenes/ICA_Classification_SingleScreenTemplate.scene'];

setenv('FSL_FIX_MCRROOT',[HCProot '/pipeline/external/fix1.06'])
setenv('CARET7DIR',[HCProot '/pipeline/workbench/bin_linux64'])
 setenv('HOME','/home/theodore/Desktop');
if(nargin<4)
...
if(slurmsub)
    ...
    system([HCProot '/pipeline/projects/Pipelines/Examples/Scripts/IcaFixProcessingBatch_pre.sh'...
        ' --StudyFolder=' outfolder ' --Subjlist=' subjid ...
        ' --runlocal --FixDir=' HCProot '/pipeline/projects/Pipelines/ICAFIX']);
    
    system([HCProot '/pipeline/projects/Pipelines/Examples/Scripts/IcaFixProcessingBatch.sh'...
        ' --StudyFolder=' outfolder ' --Subjlist=' subjid ...
        ' --runlocal --FixDir=' HCProot '/pipeline/projects/Pipelines/ICAFIX']);
    
    system([HCProot '/pipeline/projects/Pipelines/Examples/Scripts/IcaFixProcessingBatch_post.sh'...
        ' --StudyFolder=' outfolder ' --Subjlist=' subjid ...
        ' --runlocal --FixDir=' HCProot '/pipeline/projects/Pipelines/ICAFIX'])
end
...
for i=1:length(files)
    ...
    % Mode 3
   %%system(['${FSL_FIXDIR}/fix -c ' MELOUT ' ' trainingfiles ' ' num2str(thres)])
