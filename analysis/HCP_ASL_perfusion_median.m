...
if exist(fullfile(outfolder,'Perfusion_values_median.csv'),'file')
    tbl = readtable(fullfile(outfolder,'Perfusion_values_median.csv'));
else
...
if ~isempty(find(strcmp(tbl.SubjectID,subjid))) & ~force
    disp([subjid ' values are already stored in '  fullfile(outfolder,'Perfusion_values_median.csv') ' and force is false. Exiting.']);
    return
end
...
for i = 1:length(ROI)
    ...
    for j = 1:length(ROI(i).label)
        ...
        [returnval,ROI_stats] = system(['fslstats ROI.nii.gz -P 50 -V']); % Extract median (50th percentile) perfusion (-P 50) and volume (-V) for each ROI 
...
writetable(tbl, fullfile(outfolder,'Perfusion_values_median.csv'));
