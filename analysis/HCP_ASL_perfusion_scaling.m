function  HCP_ASL_perfusion(subjid,outfolder,force)

% This version of the script manually scales the CBF values in Perfusion
% according to the scaling in https://surfer.nmr.mgh.harvard.edu/fswiki/RcbfProc
% It then aligns and masks according to the T1w and extracts average ROI
% values.
...
if ~exist( fullfile(outfolder,subjid,'unprocessed','3T','Perfusion_Redo',[subjid '_3T_Perfusion.nii.gz']),'file' )
...
if exist(fullfile(outfolder,'Perfusion_values_scaled.csv'),'file')
    tbl = readtable(fullfile(outfolder,'Perfusion_values_scaled.csv'));
else
...
if ~isempty(find(strcmp(tbl.SubjectID,['''' subjid '''']))) & ~force
    disp([subjid ' values are already stored in '  fullfile(outfolder,'Perfusion_values_scaled.csv') ' and force is false. Exiting.']);
    return
end
...
copyfile( fullfile(outfolder,subjid,'unprocessed','3T','Perfusion_Redo',[subjid '_3T_Perfusion.nii.gz']) ,        fullfile(outfolder,subjid,'Perfusion') );
...
% Manually scale Perfusion nifti. Subtract 2048 then divide by 5. Threshold at
% zero.
system(['fslmaths '...
    ' ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion.nii.gz'])...
    ' -sub 2048 -div 5 -thr 0 ' ...
    ' ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_scaled.nii.gz'])...
    ])


% Rotate orientation so that FLIRT works in next step
system(['fslswapdim ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_scaled.nii.gz'])...
    ' -x -y z ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_scaled.nii.gz'])])
...
if ~exist(fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_acpc.nii.gz']),'file')
    ...
    % several minutes)
    system(['epi_reg ' ...
        ' --epi='       fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_scaled.nii.gz'])...
...
if ~exist(fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_acpc_brain.nii.gz']),'file')
    system(['fslmaths '...
        ...
        ' ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_acpc_brain.nii.gz'])...
        ]);
end
...
for i = 1:length(ROI)
    ...
    for j = 1:length(ROI(i).label)
        system(['fslmaths '...
            ...
            ' -mul ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_acpc_brain.nii.gz'])...
            ' -thr 0 '...
...
% Sort rows by subjid
tbl = sortrows(tbl, 'SubjectID');

% Write table to outfolder
writetable(tbl, fullfile(outfolder,'Perfusion_values_scaled.csv'));
