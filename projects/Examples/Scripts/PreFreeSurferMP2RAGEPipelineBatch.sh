...
for Subject in $Subjlist ; do
  ...
  echo "Found ${numT1ws} T1w MP2RAGE inv#1 Images for subject ${Subject}"
  T1wInputImages_inv1=""
  i=1
  while [ $i -le $numT1ws ] ; do
    T1wInputImages_inv1=`echo "${T1wInputImages}${StudyFolder}/${Subject}/unprocessed/3T/T1w_MPR${i}/${Subject}_3T_T1w_MP2R${i}_inv1.nii.gz@"`
    i=$(($i+1))
  done
  
  numT1ws=`ls ${StudyFolder}/${Subject}/unprocessed/3T | grep T1w_MPR | wc -l`
  echo "Found ${numT1ws} T1w MP2RAGE inv#2 Images for subject ${Subject}"
  T1wInputImages_inv2=""
  i=1
  while [ $i -le $numT1ws ] ; do
    T1wInputImages_inv2=`echo "${T1wInputImages}${StudyFolder}/${Subject}/unprocessed/3T/T1w_MPR${i}/${Subject}_3T_T1w_MP2R${i}_inv2.nii.gz@"`
    i=$(($i+1))
  done
  ...
  i=1
  while [ $i -le $numT1ws ] ; do
    T1wInputImages_uni=`echo "${T1wInputImages}${StudyFolder}/${Subject}/unprocessed/3T/T1w_MPR${i}/${Subject}_3T_T1w_MP2R${i}_uni.nii.gz@"`
    i=$(($i+1))
  done
  ...
  MagnitudeInputName="${StudyFolder}/${Subject}/unprocessed/3T/T1w_MPR1/${Subject}_3T_FieldMapMagnitude.nii.gz" #Expects 4D magitude volume with two 3D timepoints or "NONE" if not used
  PhaseInputName="${StudyFolder}/${Subject}/unprocessed/3T/T1w_MPR1/${Subject}_3T_FieldMapPhase.nii.gz" #Expects 3D phase difference volume or "NONE" if not used
  TE="2.46" #2.46ms for 3T, 1.02ms for 7T, set to NONE if not using
 ...
 echo "$T1wInputImages_inv1"
 echo "$T1wInputImages_inv2"


 ${queuing_command} ${HCPPIPEDIR}/PreFreeSurfer_MP2RAGE/PreFreeSurferPipeline_MP2RAGE.sh \
      --path="$StudyFolder" \
      --subject="$Subject" \
      --inv1="$T1wInputImages_inv1" \
      --inv2="$T1wInputImages_inv2" \
      --uni="$T1wInputImages_uni" \
      --t2="$T2wInputImages" \
