...
main()
{
    ...
    if [ ! -f ${referenceImage} ]; then
        cp ${ResultsFolder}/${g_fmri_name}.nii.gz ${ResultsFolder}/${g_fmri_name}_SBRef.nii.gz
    fi
	...
	if [ 1 == 0 ] ; then # [ 1 -eq `echo "$g_brain_ordinates_res == $g_final_fmri_res" | bc -l` ] ; then
