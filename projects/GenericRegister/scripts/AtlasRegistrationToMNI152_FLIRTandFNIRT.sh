...
# Linear then non-linear registration to T1w

${FSLDIR}/bin/flirt -interp spline -dof 12 -in ${T2wRestoreBrain} -ref ${T1wRestoreBrain}  -omat ${WD}/xfms/Generica2T1w.mat -out  ${OutputT2wImageRestoreBrain}
${FSLDIR}/bin/flirt -interp spline -dof 12 -in ${T2wRestore} -ref ${T1wRestoreBrain} -nosearch -init ${WD}/xfms/Generica2T1w.mat -out  ${OutputT2wImageRestore}
${FSLDIR}/bin/flirt -interp spline -dof 12 -in ${T2wImage} -ref ${T1wRestoreBrain} -nosearch -init ${WD}/xfms/Generica2T1w.mat -out  ${OutputT2wImage}


# Linear then non-linear registration to MNI

# T2w set of warped outputs (brain/whole-head + restored/orig)
${FSLDIR}/bin/applywarp --rel --interp=spline -i ${OutputT2wImage} -r ${Reference} -w ${OutputTransform} -o ${OutputT2wImage}
${FSLDIR}/bin/applywarp --rel --interp=spline -i ${OutputT2wImageRestore} -r ${Reference} -w ${OutputTransform} -o ${OutputT2wImageRestore}
${FSLDIR}/bin/applywarp --rel --interp=nn -i ${OutputT2wImageRestoreBrain} -r ${Reference} -w ${OutputTransform} -o ${OutputT2wImageRestoreBrain}
