...
%
%   $Header: /space/orsay/8/users/msh/CVS/CVS-MSH/MNE/mne_matlab/fiff_read_raw_segment.m,v 1.8 2006/04/23 15:29:40 msh Exp $
%   $Log: fiff_read_raw_segment.m,v $
...
if from == to
   error(me,'First and last sample cannot be the same');
end
if from > to
   error(me,'No data in this range');
end
fprintf(1,'Reading %d ... %d  =  %9.3f ... %9.3f secs...', ...
   from,to,double(from)/raw.info.sfreq,double(to)/raw.info.sfreq);
%
...
if isempty(sel)
   ...
   if isempty(raw.proj) && isempty(raw.comp)
	 ...
	 mult = raw.comp(sel,:)*raw.proj*cal;
      end
   end
end
do_debug=false;
