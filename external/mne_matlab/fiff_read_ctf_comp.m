...
%
%
%   $Header: /space/orsay/8/users/msh/CVS/CVS-MSH/MNE/mne_matlab/fiff_read_ctf_comp.m,v 1.7 2006/09/25 19:48:16 msh Exp $
%   $Log: fiff_read_ctf_comp.m,v $
...
for k = 1:length(comps)
   ...
   if ~calibrated
	 ...
	 row_cals(row) = chs(p).range*chs(p).cal;
      ...
      one.rowcals = ones(1,size(mat.data,2));
      one.colcals = ones(1,size(mat.data,1));
   end
   one.data       = mat;
   compdata(k)    = one;
   clear('row_cals');
