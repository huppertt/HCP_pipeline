function HCP_Report_Summary_Stats2(outfolder);
if(nargin<1)
...
delete(fullfile(outfolder,'/Summary/Stats/HCP_DictionaryStats.xls'));
delete(fullfile(outfolder,'/Summary/Stats/HCP_DictionaryStats.xlsx'));
for i=1:length(C); 
    disp(i);
...
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferASEGStats.xls'));
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferASEGStats.xlsx'));
Tbl2=[asegstats; wmstats];
N=unique(Tbl2.ID);
for i=1:length(N)
    lst=find(ismember(Tbl2.ID,N{i}));
    
    nirs.util.write_xls(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferASEGStats.xls'),Tbl2(lst,:),N{i});
end
...
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferStats.xls'));
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferStats.xlsx'));
N=unique(Tbl.ID);
for i=1:length(N)
    lst=find(ismember(Tbl.ID,N{i}));
    nirs.util.write_xls(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferStats.xlsx'),Tbl(lst,:),N{i}); 
end
...
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferStats_V2.xlsx'));
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferStats_V2.xls'));
tt={};
for i=1:length(Flds)
    D={};
    for i2=1:length(Subj)
        D{1,i2}=Subj{i2};
        for i3=1:length(S)
            lst=find(ismember(Tbl.ID,Subj{i2}) & ismember(Labels,S{i3}));
            D{i3+1,i2}=Tbl.(Flds{i})(lst);
        end
...
%             % so hack around it
%             tt2=tt(:,[1 tIdx:min(tIdx+254,size(D,1))]);
%             
...
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferASEGStats_V2.xls'));
delete(fullfile(outfolder,'Summary/Stats/HCP_FreeSurferASEGStats_V2.xlsx'));
