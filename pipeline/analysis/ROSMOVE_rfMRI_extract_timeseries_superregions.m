function [tc_data] = ROSMOVE_rfMRI_extract_timeseries_superregions(subjid, outfolder, force)
...
if exist(fullfile(outfolder,subjid,'MNINonLinear','Results',[subjid '_MMP_CIC_timecourses_superregions.mat']),'file') & force==false
...
for r = 1:length(runs)
    ...
    superregions = {[51 52 8 53 9 56 10 11 43],... % SM Cortex Left
        180+[51 52 8 53 9 56 10 11 43],... % SM Cortex Right
        [84 86 85 83],... % EX Cortex Left
        180+[84 86 85 83],... %EX Cortex Right
        [61 93 166 165],... % LM Cortex Left
        180+[61 93 166 165],... % LM Cortex Right
        [363 365],... % SM Subcort Left
        [371 373],... % SM Subcort Right
        [362 364],... % EX Subcort Left
        [370 372],... % EX Subcort Right
        [361],... % LM Subcort Left
        [369],... % LM Subcort Right
        [368],... % Hipp Left
        [376],... % Hipp Right
        [366],... % Thal Left
        [374],... % Thal Right
        ...
        for s = 1:length(superregions)
            l = superregions{s};
            dat_matrix_raw = NaN(max_size,size(BOLD_base,4)); % Prealloc to save time
            d = 1;
            idx = find(ismember(atlas,l));
            disp([num2str(length(idx)) '  voxels in superregion ' num2str(s)   ])
            [ilist jlist klist] = ind2sub(size(atlas),idx);
            
            for v = 1:length(ilist)
                dat_matrix_raw(d,:)   = squeeze(BOLD_base(ilist(v),jlist(v),klist(v),:));
                d = d+1;
                if d > max_size
            ...
            tc_data.raw_tc(s).dat = mean(dat_matrix_raw,1);
            %toc
        ...
        for l = 377:380
            ...
            dat_matrix_raw(any(isnan(dat_matrix_raw),2),:) = [];
            tc_data.raw_tc(l-360).dat = mean(dat_matrix_raw,1);
            %toc
...
tic
save(fullfile(outfolder,subjid,'MNINonLinear','Results',[subjid '_MMP_CIC_timecourses_superregions.mat']),'-v7.3','tc_data')
toc