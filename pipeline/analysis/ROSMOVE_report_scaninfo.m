function HCP_report_scaninfo_all(folder)
%folder='/aionraid/huppertt/XnatDB/ROS-369/';

%path(path,'/aionraid/huppertt/XnatDB/ROS-MOVE/');
...
lst2=[];
for i=1:length(s)
    scandates{i}=getScanDates(s(i).name,folder);
...
%     f=rdir(fullfile(folder,s(i).name,'unprocessed','MEG','**','*.fif'));
...
for i=1:length(Sheets)
    disp(Sheets{i});
    nirs.util.write_xls(fullfile(outfolder,'Summary','_StatsAll.xlsx'),Tbls{i},Sheets{i});
end
...
names = {'AAHScout'
    ...
    'relCBF'};
D=[];
for i=1:height(Tbls{1}); 
    for j=1:length(names); 
        D(i,j)=0;
        try
            D(i,j)=length(rdir(fullfile(Tbls{1}.MR1dcmfolder{i},[names{j} '*'],'MR*')));
        catch
            try
            D(i,j)=length(ls(fullfile(Tbls{1}.MR1dcmfolder{i},[names{j} '*'],'*')));
            end
        end
    end; 
end;
ttt=Tbls{2}.ID;
for i=1:length(names); n{i}=names{i}; n{i}(strfind(n{i},'*'))=[]; end;
ttt=[ttt array2table(D,'VariableNames',n)];

nirs.util.write_xls(fullfile(outfolder,'Summary','_StatsAll.xlsx'),ttt,Sheets{2});