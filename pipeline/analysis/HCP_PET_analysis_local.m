function HCP_PET_analysis_local(subjid,outfolder,force)
...
if(exist(fullfile(PEToutfolder,'gtm_noPVC.output','gtm.stats.dat'))~=2)
    ...
    if(exist(fullfile(PEToutfolder,'PiB-AVG.reg.lta'))~=2)
        ...
        PiBavg = [rdir(fullfile(outfolder,subjid,'unprocessed','PET',strcat(subjid,'*AVG*.nii')));...
            ...
            rdir(fullfile(outfolder,subjid,'unprocessed','PET',strcat(subjid,'*.nii.gz')))];
        
        if(length(PiBavg)==0)
            warning(['No PET data : ' subjid]);
            return;
        end
        ...
        %
        % system(['mri_convert PiB-native.nii --apply_transform /disk/HCP/analyzed/' subjid '/T1w/' subjid '/mri/transforms/talairach.lta'...
        %     ' --like /disk/HCP/analyzed/' subjid '/T1w/' subjid '/mri/T1.mgz PiB-native.nii']);
        
        if(exist('PiB-native.nii.gz')==2 & exist('Pib-native.nii')~=2)
            system('mri_convert PiB-native.nii.gz PiB-native.nii');
        end
        
        system(['mri_coreg --s ' subjid ' --mov PiB-native.nii --ref ' fullfile(outfolder,subjid,'T1w',subjid,'mri/T1.mgz')...
            ' --reg PiB-AVG.reg.lta']);
        
        %  tkregisterfv --mov PiB-native.nii --targ /disk/HCP/analyzed/HCP201/T1w/HCP201/mri/T1.mgz --reg PiB-AVG.reg.lta --s HCP201 --surfs
    ...
    system(['mri_gtmpvc --i PiB-native.nii --reg PiB-AVG.reg.lta --psf 6 --seg gtmseg.mgz' ...
        ' --default-seg-merge --auto-mask PSF .01 --mgx .01 --o gtmpvc.output --rescale 8 47 --no-reduce-fov --max-threads'])
    
    system(['mri_gtmpvc --i PiB-native.nii --reg PiB-AVG.reg.lta --psf 0 --seg gtmseg.mgz' ...
    ...
    system(['mri_convert PiB-native.nii --apply_transform PiB-AVG.reg.lta'...
...
if(exist(fullfile(PEToutfolder,'PiB_acpc_restore.nii.gz'))~=2)
    ...
    if(exist('PiB_acpc.nii.gz')~=2)
        ...
        system(['mri_convert -rt nearest -rl ' T1wImageBrain ' PiB-native.nii --apply_transform PiB-AVG.reg.lta PiB_acpc.nii.gz']);
    else
        AtlasSpaceT1wImage=fullfile(outfolder,subjid,'MNINonLinear','T1w_restore.nii.gz');
        T1wImageBrain1mm=fullfile(outfolder,subjid,'T1w','T1w_acpc_dc_restore_1mm.nii.gz');
        T1wImageBrain=fullfile(outfolder,subjid,'T1w','T1w_acpc_dc_restore.nii.gz');
    ...
    for i=1:length(files)
        ...
        if(exist(fullfile( AtlasFolder,[files{i} '_restore.nii.gz']))~=2)
            ...
            system(['mri_convert ' AtlasFolder filesep files{i} '_restore.nii ' AtlasFolder filesep files{i} '_restore.nii.gz']);
