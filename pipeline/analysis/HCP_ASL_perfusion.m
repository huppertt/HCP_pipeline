...
if exist(fullfile(outfolder,'Perfusion_values.csv'),'file')
    tbl = readtable(fullfile(outfolder,'Perfusion_values.csv'));
else
...
if ~isempty(find(strcmp(tbl.SubjectID,subjid))) & ~force
    disp([subjid ' values are already stored in '  fullfile(outfolder,'Perfusion_values.csv') ' and force is false. Exiting.']);
    return
end
...
writetable(tbl, fullfile(outfolder,'Perfusion_values.csv'));
