function  HCP_ASL_perfusion_noneg(subjid,outfolder,force)
...
if exist(fullfile(outfolder,'Perfusion_values_noneg.csv'),'file')
    tbl = readtable(fullfile(outfolder,'Perfusion_values_noneg.csv'));
else
...
if ~isempty(find(strcmp(tbl.SubjectID,subjid))) & ~force
    disp([subjid ' values are already stored in '  fullfile(outfolder,'Perfusion_values_noneg.csv') ' and force is false. Exiting.']);
    return
end
...
if ~exist(fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_acpc_brain_noneg.nii.gz']),'file') | force
    system(['fslmaths '...
        ...
        ' -thr 0 '...
        ' ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_acpc_brain_noneg.nii.gz'])...
        ]);
end
...
for i = 1:length(ROI)
    ...
    for j = 1:length(ROI(i).label)
        system(['fslmaths '...
            ...
            ' -mul ' fullfile(outfolder,subjid,'Perfusion',[subjid '_3T_Perfusion_acpc_brain_noneg.nii.gz'])...
...
writetable(tbl, fullfile(outfolder,'Perfusion_values_noneg.csv'));
