...
get_batch_options() {
    ...
    unset command_line_specified_fsflags
    ...
    while [ ${index} -lt ${numArgs} ]; do
        ...
        case ${argument} in
            ...
            --runlocal)
                ...
                ;;
            --flags=*)
                command_line_specified_fsflags=${argument/*=/""}
                index=$(( index + 1 ))
                ;;
        esac
    done
...
if [ -n "${command_line_specified_fsflags}" ]; then
    fsflags="${command_line_specified_fsflags}"
fi
