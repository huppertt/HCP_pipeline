...
HCPdir="/aionraid/huppertt/raid2_BU/HCP"
# Global default values
DEFAULT_STUDY_FOLDER="${HCPdir}/analayzed"
DEFAULT_SUBJECT_LIST="100307"
DEFAULT_ENVIRONMENT_SCRIPT="${HCPdir}/pipeline/projects/Pipelines/Examples/Scripts/SetUpHCPPipeline.sh"
DEFAULT_RUN_LOCAL="TRUE"
DEFAULT_FIX_DIR="${HCPdir}/pipeline/projects/Pipelines/ICAFIX"
		...
		do
			...
			do
                ...
                if [ -f $InputFile ] ; then
                    ...
                    if [ "${RunLocal}" == "TRUE" ]
                    then
                        echo "About to run ${FixDir}/hcp_fix_pre2 ${InputFile} ${bandpass}"
                        queuing_command=""
                    else
                    ...
                    ${queuing_command} ${FixDir}/hcp_fix_pre2 ${InputFile} ${bandpass}
                fi
			done
