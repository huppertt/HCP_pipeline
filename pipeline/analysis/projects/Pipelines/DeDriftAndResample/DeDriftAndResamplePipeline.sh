...
main()
{
	...
	for Mesh in ${LowResMeshes} ${HighResMesh} ; do
        ...
        if [ -f ${AtlasFolder}/T2w_restore.nii.gz ] ; then
       ...
       #SphericalDistortion
		for Map in ${Maps} ${MyelinMaps} ArealDistortion EdgeDistortion ; do
			log_Msg "Map: ${Map}"
			...
			for MapMap in ${MyelinMaps} ; do
				...
				if [ ${MapMap} = ${Map} ] ; then
                    ...
                    if [ -f ${NativeFolder}/${Subject}.MyelinMap.native.dscalar.nii ] ; then
                    ...
                    ${Caret7_Command} -cifti-math "Individual - Reference" ${DownSampleFolder}/${Subject}.BiasField_${ConcatRegName}.${LowResMesh}k_fs_LR.dscalar.nii -var Individual ${DownSampleFolder}/${Subject}.MyelinMap_${ConcatRegName}.${LowResMesh}k_fs_LR.dscalar.nii -var Reference ${DownSampleFolder}/${Subject}.MyelinMap_BC.${LowResMesh}k_fs_LR.dscalar.nii
            ...
            if [ -f  ${Folder}/${Subject}.MyelinMap_BC.${Mesh}k_fs_LR.dscalar.nii ] ; then
	...
	for Map in ${MyelinMaps} ; do
        if [ -f ${NativeFolder}/${Subject}.${Map}_BC_${ConcatRegName}.native.dscalar.nii ] ; then
	...
	for fMRIName in ${rfMRINames} ; do
        ...
        #echo "${HCPPIPEDIR}/ReApplyFix/ReApplyFixPipeline.sh --path=${StudyFolder} --subject=${Subject} --fmri-name=${fMRIName} --high-pass=${HighPass} --reg-name=${ConcatRegName}"
	done
