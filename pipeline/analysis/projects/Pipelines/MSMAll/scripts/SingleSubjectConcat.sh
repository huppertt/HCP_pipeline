...
source /aionraid/huppertt/raid2_BU/HCP/pipeline/projects/Pipelines/global/scripts/log.shlib # Logging related functions
log_SetToolName "${g_script_name}"

MATLAB_HOME=/aionraid/huppertt/raid2_BU/HCP/pipeline/external/Matlab/v81
	...
	if [[ ! -e ${OutputConcat} || `echo ${g_migp_var} | cut -d "@" -f 4` = "YES" ]]; then
		...
		case ${g_matlab_run_mode} in
			0)
				...
				matlab_compiler_runtime=/disk/HCP/pipeline/external/Matlab/v81/
