...
if~(exist(fullfile(outfolder,subjid,'T1w',subjid,'dmri','HCP-MMP_subcort_atlas.nii.gz'),'file') & ~force)
    ...
    system(['rm -v ./WMH.nii.gz']);              % ACPC lesion mask (this needs to be generated by Minjie Wu's code)
    system(['rm -v ./subcortical.nii.gz']);      % Subcortical atlas
    system(['rm -v ./HCP-MMP.nii.gz']);          % Custom volumetric HCP-MMP atlas
    ...
    system(['cp -v ' fullfile(outfolder,subjid,'T2FLAIR',[subjid '_3T_WMH_fuzzy_acpc.nii.gz'])             ' ./WMH.nii.gz']);              % ACPC lesion mask (this needs to be generated by Minjie Wu's code)
    system(['cp -v ' fullfile(outfolder,subjid,'T1w','subcortical','subcortical_all_fast_firstseg.nii.gz') ' ./subcortical.nii.gz']);      % Subcortical atlas
    system(['cp -v ' fullfile(outfolder,subjid,'T1w',['HCP-MMP_labels_acpc.nii.gz'])                       ' ./HCP-MMP.nii.gz']);          % Custom volumetric HCP-MMP atlas
    ...
    system(['flirt -in subcortical.nii.gz -ref HCP-MMP.nii.gz -out subcortical.nii.gz -nosearch -applyxfm -init ' fsldir '/etc/flirtsch/ident.mat -interp nearestneighbour']); % Resample subcortical to atlas resolution; overwrite subcortical
...
if(~exist([ subjid '_dsistudio.fib.gz']) || force)
    ...
    system([dsiroot filesep 'dsi_studio --action=rec --source=' subjid '_dsistudio.src.gz '...
        ...
        ' --other_image=t1w:T1w_brain.nii.gz,lesion:WMH.nii.gz']);
    %
...
% Compute pairwise ROI tract connectivity stats (Note: We will want a
% custom atlas here: HCP-MMP_subcort_atlas.nii.gz
...
if(~exist([subjid '_dsistudio.trk.gz.HCP-MMP_subcort_atlas.count.end.connectivity.mat'],'file')...
    ...
    system([dsiroot filesep 'dsi_studio --action=ana --source=' subjid '_dsistudio.fib.gz '...
        ...
        ' --connectivity=FreeSurferDKT,HCP-MMP,Gordan_rsfMRI333 '...
